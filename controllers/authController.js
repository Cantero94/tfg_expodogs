...
const registrarUsuario = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ mensaje: "Registro exitoso. Revisa tu correo para activarlo." });
  } catch (error) {
...
const verificarCuenta = async (req, res) => {
  try {
    ...
    req.session.mensaje =
      "Su cuenta ha sido activada correctamente. ¡Bienvenido!";
    ...
    if (!email || !password) {
      ...
      if (!usuario) {
        ...
        if (!passwordMatch) {
          ...
          // 🔐 Si el usuario marcó "mantener sesión iniciada"
          if (remember === "on") {
            // 30 días
...
const recordarPassUsuario = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ mensaje: "Revisa tu correo para restablecer tu contraseña." });
  } catch (error) {
...
const restablecerPassword = async (req, res) => {
  try {
    ...
    req.session.mensaje =
      "Tu contraseña ha sido restablecida a '123456'. Te recomendamos cambiarla en el apartado Mi Cuenta.";
    return req.session.save(() => res.redirect("/"));
  } catch (error) {
